...
class Dial {
  ...
  Dial(int aix, float ax, float ay) {
    ...
    setOTextBox.mk(textboxix, int(l-30), int(t-29), 50, 25);
    tbix[0] = textboxix;
    textboxix++;
    setOTextBox.mk(textboxix, int(l-30+55), int(t-29), 50, 25);
    tbix[1] = textboxix;
    textboxix++;
    setOTextBox.mk(textboxix, int(l-30+110), int(t-29), 50, 25);
    tbix[2] = textboxix;
    textboxix++;
  }
  ...
  void msdrg() {
    ...
    if ((edittog%2)==0 && drag) {
      ...
      val = map(val, 0.0, 1.0, vlo, vhi);
      osc.send(oscheaderloc + "/dial1", new Object[] {
        val
      }
      , oscdest);
    }
  ...
  void setdval(float inval) {
    val = map(inval, 0.0, 1.0, 0.0, TWO_PI);
    drotate = val;
    if (crv<0) val = pow(val, 1.0/abs(crv));
    else val = pow(val, crv);
    val = map(val, 0.0, 1.0, vlo, vhi);
